<template>
  <div class="container-x py-6">
    ...
    <div v-if="!userStore.isAdmin" class="mt-10 card p-10 text-center">
      ...
      <h2 class="mt-3 text-xl font-bold text-red-600">Access Denied</h2>
      <p class="text-gray-500 mt-1">Staff / Admin only.</p>
    </div>

    <div v-else class="mt-4">
      <h1 class="text-2xl font-bold text-brand-500 mb-4">Admin Dashboard</h1>
      ...
      <div class="grid gap-4 grid-cols-2 md:grid-cols-4 mb-6">
        ...
        <div class="card p-5">
          <div class="text-xs text-gray-500">Revenue (USD)</div>
      ...
      <div class="flex gap-1 mb-4 border-b border-gray-200">
        <button v-for="tab in ['inquiries', 'users']" :key="tab" @click="active = tab"
                :class="['px-4 py-2 text-sm font-medium border-b-2 -mb-px',
                         active === tab ? 'border-brand-500 text-brand-500' : 'border-transparent text-gray-500 hover:text-gray-700']">
          {{ tab === 'inquiries' ? 'Inquiries' : 'Users' }}
        </button>
      </div>
        ...
        <table class="w-full text-sm">
          ...
          <tbody>
            <tr v-for="i in inquiries" :key="i.id" class="border-t border-gray-100">
              ...
              <td class="px-3 py-2">
                <button class="text-xs text-accent hover:underline">View</button>
              </td>
            </tr>
          </tbody>
        </table>
